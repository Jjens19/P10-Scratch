...
# Empty bytes flow after first state
if test_id >= 10: 
    process.stdin.write(f"bytes logged\n")
    process.stdin.flush()
...
for step in range(steps_max): # ----------------------------------------STEP-----------------------------------------
    ...
    # CODE FOR FETCHING CUBIC CWND IN TEST 10 AND ABOVE
    if test_id >= 10:
        # Get cWnd value from file
        with open("scratch/cwnd_value.txt", 'r') as file:
            cwnd_list[1].append(float(file.read()))
        
        process.stdin.write(f"bytes logging\n")
        process.stdin.flush()

        # Read value from buffer and run send message to run more code
        reading = process.stdout.readline()
        tcp_bytes = reading.split('\n')[0]
        received_bytes_list[1].append(float(tcp_bytes))
...
plot_data_over_time(data_lists_list=   [[cwnd_list[0]],[received_bytes_list[0]],[custom_bytes_list_sum],
                    ...
                    filepath=          f"scratch/results/")

import pickle

data = [cwnd_list, received_bytes_list, [custom_bytes_list_sum, cubic_bytes_list_sum]]
with open(f"scratch/results/data_model_{model_id}_training_{training_n}_test_{test_id}", "wb") as file:
