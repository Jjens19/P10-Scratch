...
void RttChangeCallback(Time oldRtt, Time newRtt) {
    std::cout << "RTT changed from: " << oldRtt.GetMilliSeconds()
              << "ms to: " << newRtt.GetMilliSeconds() << "ms" << std::endl;
}
...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        NS_LOG_UNCOND(total << "," << g_packetCount << "," << g_ackReceived << "," << g_bytesSent);

    } while (g_packetCount > 0 || g_ackReceived > 0 || g_bytesSent > 0 || total < simLength);
